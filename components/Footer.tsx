export default function Footer() {
  const sections = [
    { title: "Services", items: ["Branding", "Design", "Marketing", "Advertisement", "Press Kit"] },
    { title: "Company", items: ["About Us", "Contact", "Jobs", "Blog", "Press Kit"] },
    { title: "Support", items: ["FAQs", "Help Center", "Community", "Guides", "Contact Support"] },
  ];

  const bottomLinks = ["Terms of Use", "Privacy Policy", "Cookie Policy"];

  return (
    <footer className="bg-cyan-800 text-gray-100">
      {/* Top Section */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
        {sections.map((section) => (
          <div key={section.title}>
            <h6 className="text-lg font-medium mb-3 font-metro">{section.title}</h6>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex flex-col md:flex-row justify-between items-center text-sm text-blue-100">
        <p className="mb-4 md:mb-0 text-center md:text-left">
          <span className="font-semibold text-gray-300">PawsRescue</span> <br />
